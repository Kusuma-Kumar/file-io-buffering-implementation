...
typedef struct fileInfo {
    ...
    int reading;     // Flag to indicate if the current operation is reading
} MYFILE;

struct fileInfo *myopen(const char* pathname, int flags);
void myread(struct fileInfo *file, void *buf, size_t nbyte);
void myseek(struct fileInfo *file, off_t offset, int whence);
